...
| <span style={{color:'green'}}>**authsamples.com**</span> | The root domain will host interstitial web pages used during native app logins |
...
There are four essential behaviours that the CDN host needs to provide, summarized in the following table:
...
The most interesting setting is the [Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) which I set to this string to protect against common threats and to only allow the SPA’s JavaScript to make remote calls for its web origin and API endpoints:
