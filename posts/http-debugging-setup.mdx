...
I use my own wrapper scripts with mitmproxy, so that I can quickly get set up on a Windows, macOS or Linux computer, without manual steps. You can clone the repo with this command:
...
- Download the mitmproxy root certificate and add it to the system trust store

It then continues to perform the following actions:

- Run the mitmweb process
- Activate mitmproxy as the system proxy
- Open mitmweb in the default browser
...
Next I proceed to debug HTTP messages for the components of interest. In some cases, extra configuration steps may be necessary to trace particular messages. The details are explained in the following sections. Finally, when I have finished debugging HTTP requests, I run the *stop.sh* script to free resources.
