...
The SPA is deployed to many global locations using AWS CloudFront. Anyone can run the app via the below link:

- https://www.authsamples.com

Alternatively, you can run the app's code locally and connect it to cloud-hosted API endpoints by following the <Link href='/posts/how-to-run-the-react-js-spa'>Final SPA Instructions</Link>. In either case, login with one of the test accounts:
...
A Private URI Scheme is used to notify the app of the login result and return it to the foreground. To run the app on your local PC you can follow the <Link href='/posts/final-desktop-sample-overview'>Final Desktop App Instructions</Link>.
...
Logins are managed via a *Chrome Custom Tab*, with modern security and good usability. To run the app from Android Studio you can follow the <Link href='/posts/basicandroidapp-execution'>Android Code Sample Instructions</Link>.
...
![iOS view](/images/3/ios-view.jpg?v=20240713)

Logins are managed via Apple’s *ASWebAuthenticationSession* window. To run the app from Xcode you can follow the <Link href='/posts/how-to-run-the-ios-code-sample'>iOS Code Sample Instructions</Link>.

![iOS login](/images/3/ios-login.jpg?v=20240713)
...
- <Link href='/post/api-journey-server-side'>API Journey – Server Side</Link>
