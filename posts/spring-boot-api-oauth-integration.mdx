...
| <span style={{color:'green'}}>**Supportable**</span> | Identity and error details will be captured and included in logs, and JSON error responses will be customizable. |
...
| <span style={{color:'green'}}>**Title**</span> | A business title for the end user, which is displayed by frontend applications |
...
When the OAuth filter runs successfully, it update the holder object with the claims principal. When the controller logic runs, the claims principal can be resolved, and this works even if the request uses child threads. The API can then easily implement its authorization logic:
...
- Next we will describe other details of our <Link href='/posts/spring-boot-api-coding-model'>Java API Coding Key Points</Link>
