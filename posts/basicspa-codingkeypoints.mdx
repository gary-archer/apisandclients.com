...
The SPA uses the following types of static resource that are downloaded to the browser. In this blog I use only basic HTML and CSS, so that the main code focus for SPAs is the JavaScript logic:
...
The SPA's concerns are separated into a number of TypeScript classes, and in particular we keep plumbing separated from the application's more valuable business logic:
...
The [Bootstrap Grid System](https://getbootstrap.com/docs/4.0/layout/grid/) is used to lay out elements at runtime, and the HTML we deploy is minimal:
...
As the user navigates between screens, a main element within the root element is updated with a different view.

### SPA Entry Point

When our SPA’s *index.html* page loads, it creates a global instance of an application class defined in the *App.ts* file, and calls the *execute* method on that object:
...
The SPA configuration contains the following values, to enable it to connect to the API and to perform OAuth login redirects:
...
The *ApiClient* asks the *Authenticator* for an access token so that it can make the API request. If it cannot get one, the *ApiClient* triggers a login redirect. The in-flight API call is also terminated, with a *login_required* error. The SPA’s error handling code ignores this error code, which prevents any error details from being rendered.
...
- Removes the authorization response from the browser history
...
The above code demonstrates *deep linking*, where a user bookmarks a location like `http://localhost/spa/#company=2`, then access it in a new browser session. After signing in, the user is returned to their bookmarked location.
...
The API credential is a *Bearer Token*, and if an attacker can somehow intercepts one they can also send it to the API. To mitigate this threat, always use least privilege and short-lived access tokens.

### Safe Input Handling

The SPA uses the [Mustache Template Library](https://github.com/janl/mustache.js/) to bind received data to the SPA's DOM elements. This ensures that potentially dangerous input is escaped, to prevent malicious code execution. This blog's final SPA uses React, which provides similar input protection.
