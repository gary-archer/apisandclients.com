...
Ideally we would like an equivalent setup to our earlier APIs, where we listen for HTTP requests on a developer PC, and can run a frontend client against the API:

![spa and api](/images/620/spa-and-api.jpg)
...
The Serverless API project is available [here](https://github.com/gary-archer/authguidance.apisample.serverless) and can be downloaded / cloned to your local PC with this command:
...
The API serves this blog’s final frontend code samples which you can run as explained in the <Link href='/posts/quick-start'>Quick Start</Link> page. Each frontend has its own client best practices. The SPA uses the latest HTTP-only secure cookies when calling the Serverless API.
