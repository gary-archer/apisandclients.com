...
When you need to update certificate trust, you can either add the entire trust bundle or you can trust just a particular root CA. The following sections show where trust is added.
...
On macOS we instead add the root certificate(s) to the *Keychain Access* application under *System / Certificates*, then enable the *Always Trust* option:
...
```bash
sudo cp myroot.ca.crt /usr/local/share/ca-certificates/
...
Later in this blog I provide an <Link href='/posts/java-spring-boot-api-overview'>OAuth Secured Sample Java API</Link>, and each Java installation has its own certificate trust store in a *cacerts* file under the Java install location. For example, on macOS this is at the following location when running Java 21:
...
Later in this blog I also provide some <Link href='/posts/desktop-apps-overview'>OAuth Secured Desktop Apps</Link>, where there are two processes, which determine certificate trust with different rules:
