...
The deep linking domains allowed by the app are configured as *Associated Domains* in a *basicmobileapp.entitlements* file:
...
![ios debug success](/images/560/ios-debug-success.jpg)

Yet some developers may experience errors when running the app. A failed registration results in output like this:

![ios debug failure](/images/560/ios-debug-failure.jpg)

### Fixing Deep Linking Registration Failures
...
Yet this is not really how my code sample is meant to be run. An alternative option is to host your own deep linking assets file on a development web server. To do so, start by updating the */etc/hosts* file on your MacBook, to alias the deep linking domain to localhost:

```text
127.0.0.1 mobile.authsamples.com
```

Then run the following script, providing your Apple Team ID:
...
You can study the script to see how it creates an SSL certificate and key, registers the root certificate authority as trusted on the simulator, then runs an HTTPS web server, to serve a deep linking assets file that uses your Apple Team ID. Finally, configure Xcode to use the same Apple Team ID to sign the app before deploying it to the simulator:

![code signing](/images/560/code-signing.jpg)

Since we are using [mode=developer](https://developer.apple.com/documentation/bundleresources/entitlements/com_apple_developer_associated-domains) the app should then run successfully. This technique will only work on simulators, and for a real app you would need to provide your own iOS infrastructure to run on devices.
