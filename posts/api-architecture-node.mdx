...
Previously we drilled into this blog’s <Link href='/posts/error-handling-and-supportability'>Error Handling and Supportability</Link> design. Next we will explain this blog’s final Node.js API. See also the <Link href='/posts/api-journey-server-side'>API Journey – Server Side</Link> blog post for the API’s main behaviours.
...
![spa and api](/images/300/spa-and-api.jpg?v=20240713)

### API URLs

During development the API is run as a standalone component. Either mocha tests or a load test can be used as API clients. These tests can get user level access token to call the API and verify its OAuth security. This is done by pointing the API to a mock authorization server:
...
First update the hosts file to create domains for local development. Include the web domain and local backend for frontend domain if running the SPA and API together:
...
Download [Node.js](https://nodejs.org/en/download) for your operating system, which will be used to run the API and also to build and host the SPA’s JavaScript bundles.
...
The API sample is available [here](https://github.com/gary-archer/oauth.apisample.nodejs) and can be be downloaded / cloned to your local PC:
...
![running api](/images/300/running-api.jpg)

### Step 2: Configure SSL Trust

The API runs over SSL using a development certificate. To prevent SSL trust problems from tests or the browser you may need to <Link href='/posts/developer-ssl-setup'>Configure Node.js and Browser Trust</Link> for the highlighted root certificate below:

![certs](/images/300/certs.jpg)

You will then be able to navigate to the API URL in a browser without any SSL warnings:

![browse to api](/images/300/browse-to-api.jpg)
...
For further details on testing the API with user level tokens but without running a full user login flow, see the  <Link href='/post/api-automated-tests'>API Automated Tests</Link> blog post.

### Step 5: Run the SPA Client

You can also run the <Link href='/posts/how-to-run-the-react-js-spa'>Final SPA</Link> as a client of the API, using the following commands, where SPA resources are downloaded to a parallel folder:
...
The OAuth integration and its claims handling uses exactly the same techniques as the second code example, summarised earlier in its <Link href='/posts/updated-api-coding-key-points'>API Coding Key Points</Link>. Therefore I will not repeat them, and these posts will instead only focus only on newer features.
...
- Next we will take a look at the <Link href='/posts/final-nodeapi-coding-key-points'>Node.js API Coding Key Points</Link>
