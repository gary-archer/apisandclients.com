...
In the previous post we described the <Link href='/posts/spring-boot-api-coding-model'>Java Spring Boot API Coding Key Points</Link>. Next we will describe one way to test OAuth secured APIs, with a focus on *Developer Productivity*.
...
| <span style={{color:'green'}}>**Client Application Name**</span> | The name of the load test, as a mechanism to find all load test sessions |
