...
Previously we provided an explanation of the <Link href='/posts/spa-back-end-for-front-end'>Backend for Frontend</Link> setup  for the final SPA, where remote API components issue cookies for the SPA. Next we will summarize the main frontend behaviours of the completed SPA, then get it running locally.
...
| ----------- | ----------- |
| <span style={{color:'green'}}>**Best Browser Security**</span> | Only the strongest *HTTP-only SameSite=strict* cookies will be used, with no tokens in the browser |
...
As explained in the previous post, the following components are used by the SPA to implement end-to-end security. The security is based on the [Token Handler Example](https://github.com/curityio/spa-using-token-handler), where the OAuth agent handles cookie issuing on behalf of the SPA:

![components](/images/220/components.jpg)
...
![online spa](/images/220/online-spa.jpg)
...
![oauth client production](/images/220/oauth-client-production.jpg)
...
![oauth client development](/images/220/oauth-client-development.jpg)
...
![mobile view](/images/220/mobile-view.jpg)
...
Some companies run multiple micro-frontends for the same business area within a single web domain. This keeps web code manageable as it grows and also provides a bundle splitting solution.

The final web code example is hosted at an */spa* path to show that other apps could be added alongside it, and share the same cookies. This would enable multiple teams to each work on only one frontend at a time, and avoid loading the code for all other apps. Each frontend would call APIs  using shared cookies, and it would be possible to seamlessly navigate across apps without OpenID Connect redirects.

### Legal and Regulatory

A real cookie secured SPA may also need to ask the user to consent to use of cookies, and provide information on how any personal information is used, to comply with regulations such as [GDPR](https://gdpr.eu/cookies). This blog’s SPA omits this step, since it uses only fictional data and fictional user accounts.
