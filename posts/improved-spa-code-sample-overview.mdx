...
The *authsamples-dev.ca.pem* file is a root certificate authority that must be added to your system trust store in order for the browser to use SSL without warnings. See the <Link href='/posts/developer-ssl-setup'>Browser SSL Trust</Link> instructions for further information.
