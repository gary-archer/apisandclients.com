...
First configure and run the HTTP proxy on the host computer, as described in our earlier write up on <Link href='/posts/http-debugging-setup'>HTTPS Debugging</Link>. We also need to instruct the proxy to decrypt SSL traffic, as covered in our page on <Link href='/posts/developer-ssl-setup'>SSL Trust Configuration</Link>:
...
From an emulator or device you can open Chrome and type the IP address of the host. Click a certificate file to save it to the device’s *Downloads* folder:
