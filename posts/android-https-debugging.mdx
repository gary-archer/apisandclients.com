...
Previously we enabled an initial Android setup by running the <Link href='/posts/android-setup'>AppAuth Android Code Sample</Link>. Next I will explain a way to view HTTPS mobile traffic from iOS simulators and devices on a development computer.

### Run the HTTP Proxy on the Host

First configure and run the HTTP proxy on the host computer, as described in our earlier write up on <Link href='/posts/http-debugging-setup'>HTTPS Debugging</Link>. We also need to instruct the proxy to decrypt SSL traffic, as covered for other environments in the blog post on <Link href='/posts/developer-ssl-setup'>SSL Trust Configuration</Link>:
...
![network proxy](/images/470/network-proxy.jpg?v=20240804)
...
![cellular off](/images/470/cellular-off.jpg?v=20240804)
...
![cellular apn settings](/images/470/cellular-apn-settings.jpg?v=20240804)
...
![ssl trust error](/images/470/ssl-trust-error.jpg?v=20240804)
...
![proxy root ca](/images/470/proxy-root-ca.jpg?v=20240804)

### Deploy the HTTP Proxy Root Certificate

To resolve the trust error you must get the certificate's root certificate authority file to the emulator or device. This can be down in various ways, such as by emailing the file. For the MITM proxy root CA you can just browse to a URL of *http://mitm.it* from the device or emulator.
...
![get proxy root ca](/images/470/get-proxy-root-ca.jpg?v=20240804)
...
![user credentials](/images/470/user-credentials.jpg?v=20240804)
...
On all simulators and devices we can now view OAuth and API HTTPS messages, to view OAuth-related requests, such as those produced by the AppAuth library:
