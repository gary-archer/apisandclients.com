...
Previously we described our <Link href='/posts/android-infrastructure-setup'>Android Infrastructure Setup</Link> and we will now look at some key areas of our sample’s code. See also the <Link href='/posts/api-journey-client-side'>Client Side API Journey</Link> to understand the background and the requirements being met.

### Goal: Portable Coding Model

One of this blog’s coding goals is to use the same UI classes across multiple platforms. Our Android app uses the same logical separation of responsibilities into classes as the earlier <Link href='/posts/reactjs-codingkeypoints'>React SPA</Link>:

![code layout](/images/530/code-layout.jpg)
...
![view classes](/images/530/view-classes.jpg)
...
Use of [Kotlin Coroutines](https://developer.android.com/kotlin/coroutines) results in a readable async await coding model where we write simple suspending functions to do the work. The code also caches API responses to prevent redundant calls when views are recreated, such as during back navigation.
...
![concurrent views](/images/530/concurrent-views.jpg)
...
![login completion](/images/530/login-completion.jpg)
...
![cancelled logins](/images/530/cancelled-logins.jpg)
...
![login required](/images/530/login-required.jpg)
...
When multiple views call APIs and receive 401 responses, the token renewal call should be *synchronized* so that it only occurs once. If we view HTTP traffic we can see the correct behaviour:
...
![synchronized refresh](/images/530/synchronized-refresh.jpg)
...
Logout request messages include a *Post Logout Redirect URI* that points to our *Web Hosted Post-Logout Page*:

![logout completion](/images/530/logout-completion.jpg)
...
![appauth error](/images/530/appauth-error.jpg)
...
The app can be debugged via the below toolbar icon, to step through code using commands from the *Run Menu*. This also enables us to view the state of AppAuth objects, when we need to troubleshoot OAuth code:

![debug](/images/530/debug.jpg)
...
![code quality warnings](/images/530/code-quality-warnings.jpg)
...
- Next we will start describing our <Link href='/posts/ios-code-sample-overview'>Secured iOS App’s Behaviour</Link>
