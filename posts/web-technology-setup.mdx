...
| [allowSyntheticDefaultImports](https://www.typescriptlang.org/tsconfig#allowSyntheticDefaultImports) | Enable the cleanest import syntax for CommonJS modules |
...
For SPAs, we need to ensure that output works in older browsers, but we also want to produce clean production JavaScript code, without the need for excessive [polyfilling](https://javascript.info/polyfills). The following *tsconfig.json* output settings are used:
...
During development of TypeScript APIs, I prefer to avoid producing JavaScript files, since doing so adds confusion when searching the code base.
